document.addEventListener('DOMContentLoaded', function() {
  ...
  var bookingExtraInfo = document.getElementById('bookingExtraInfo');
  var comboLabel = document.getElementById('comboLabel');

  if (bookingPopup && bookingForm) {
    bookingBtns.forEach(function(btn) {
      btn.onclick = function(e) {
        ...
        // Hiển thị thêm thông tin ưu đãi nếu có
        if (bookingExtraInfo) {
          var desc = btn.dataset.desc ? `<div><b>Mô tả:</b> ${btn.dataset.desc}</div>` : '';
          var price = btn.dataset.price ? `<div><b>Ưu đãi:</b> ${btn.dataset.price}</div>` : '';
          var time = btn.dataset.time ? `<div><b>Hiệu lực:</b> ${btn.dataset.time}</div>` : '';
          bookingExtraInfo.innerHTML = desc + price + time;
        }
  ...
  if (bookingService && comboLabel) {
    bookingService.onchange = function() {
      if (bookingService.value === 'Combo PS5 + NET') {
